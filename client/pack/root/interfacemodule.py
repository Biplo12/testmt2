...
class Interface(object):
	...
	def __CheckGameMaster(self, name):
		if not self.IsGameMasterName(name):
			return
	...
	def IsGameMasterName(self, name):
		if self.listGMName.has_key(name):
			return True
		cleanName = name.replace("|cFFFFA200", "").replace("|cFFFF0000", "").replace("|cFF0000FF", "").replace("|cFF3296FF", "")
		if cleanName.startswith("[GM]") or cleanName.startswith("[MOD]"):
			return True
		return False
